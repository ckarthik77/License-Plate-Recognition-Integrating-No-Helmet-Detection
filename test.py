...
# Load both models
plate_model = YOLO("lprbest.pt")      # License Plate Detection
helmet_model = YOLO("best.pt")        # Helmet Detection ("With Helmet", "Without Helmet")
...
def process_webcam(output_file="output_live.avi"):
    ...
    # Set desired resolution (if supported by webcam)
    target_width = 1280
    target_height = 720
    ...
    while True:
        ...
        cv2.imshow("Helmet & Number Plate Detection - Webcam", annotated_frame)
    ...
    logging.info(f"Live webcam processing finished. Output saved as {output_file}")
